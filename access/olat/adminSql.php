...
if (isset($_POST['action'])) {
	if ($_POST['action'] == 'addTest') {
		...
		if ($result) {
   		...
   		<div class="card-body">
				<a href="#" class="atag testInstruction" data-test="' . $id . '"><i class="fa fa-edit"></i></a>
			...
			for ($i = 1; $i <= $test_section; $i++) {
				...
				echo '<div class="row">';
				echo '<div class="col"><button class="btn btn-info btn-square-sm btn-block sectionInstruction" data-test="' . $id . '" data-section="' . $i . '">Section : ' . $i . ' Instructions</button></div>';
				echo '</div>';
			}
			echo '</div></div>';
   		...
   		<div class="card-body">';
			echo '<div class="row">';
			echo '<div class="col-12"><button class="btn btn-primary btn-sm btn-block studyMaterial"> Study Material</button></div>';
			echo '</div>';
			echo '</div></div>';
		}
...
} elseif ($_POST['instructionId'] == 'T' || $_POST['instructionId'] == 'S' || $_POST['instructionId'] == 'M') {
	$test_id = $_POST['testId'];
	...
	elseif ($id == "S") $fileName = $folder . '/instructions_section' . $sectionId . '.txt';
	elseif ($id == "M") {
		$folder = '../../' . $myFolder . '/test';
		if (!is_dir($folder)) mkdir($folder);
		$sql="insert into resource_tag (rtag_name) values('".$_POST['tag']."')";
		$conn->query($sql);
