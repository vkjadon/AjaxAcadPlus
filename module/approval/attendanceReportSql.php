...
if (isset($_POST['action'])) {
	if ($_POST['action'] == 'sasEACReport') {
		...
		//echo " Class $classId $tn_rc";
		$sql = "select std.* from $tn_rc rc, student std where rc.class_id='$classId' and rc.student_id=std.student_id";
		$json = getTableRow($conn, $sql, array("student_id", "student_name", "student_rollno"));
		//echo $json;
		$array = json_decode($json, true);
		//echo $array;
		echo '<table class="table list-table-xxs">';
		echo '<tr align="center"><th>Id</th><th>Student</th><th>RollNo</th><th>Extra Attd</th><th>Action</th></tr>';
		for ($i = 0; $i < count($array["data"]); $i++) {
			$student_id = $array["data"][$i]["student_id"];
			echo '<tr>';
			echo '<td>' . $student_id . '</td>';
			
			echo '<td>' . $array["data"][$i]["student_name"] . '</td>';
			echo '<td>' . $array["data"][$i]["student_rollno"] . '</td>';
			echo '<td align="center"><button class="btn btn-info btn-square-sm mt-1 detailButton" data-std="' . $student_id . '">Details</button></td>';
