...
if (isset($_POST['action'])) {
  if ($_POST['action'] == 'fetchStudentAutoList') {
    ...
    // echo "$student_id $ft";
    if ($myId > 0) {
      $sql = "insert into fee_receipt (fr_sno, fee_type, fr_date, student_id, fee_mode, fee_semester, fr_amount, transaction_id, transaction_date, fr_bank, fr_desc, update_id, fr_status) values ('$fr_sno', '$ft', '$feeDate', '$student_id', '$fm', '$sem', '$fee','$t_id', '$transaction_date', '$fr_bank', '$feeDesc', '$myId', '0')";
      $result = $conn->query($sql);
      if (!$result) echo $conn->error;
