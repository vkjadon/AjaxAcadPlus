...
if (isset($_POST['action'])) {
    if ($_POST['action'] == 'staffList') {
        ...
        $statusDecode = array("status" => "staff_status", "0" => "Active", "9" => "Inactive");
        $button = array("1", "1", "0", "0", "Details");

        $fields = array("staff_name", "staff_mobile", "staff_email", "designation_name");
        $dataType = array("0", "0", "0", "0", "0");
        $header = array("Id", "Staff Name", "Staff Mobile", "Staff Email", "Designation");

        if ($dept_id > 0) $sql = "select st.*, d.designation_name from staff st, designation d where d.designation_id=st.designation_id and st.dept_id='$dept_id' order by st.staff_id";
