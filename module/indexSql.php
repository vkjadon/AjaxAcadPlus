...
if (isset($_POST['action'])) {
   if ($_POST['action'] == 'profile') {
      // $id = $_POST['userId'];
      // echo $myId;
      $sql = "select s.staff_name, s.mn_id, s.dept_id, s.staff_mobile, s.staff_email, s.staff_doj, s.staff_image from staff s where s.staff_id='$myId'";
      $result = $conn->query($sql);
      $jasonArray=array();
      if($result && $result->num_rows>0){
         $rowArray = $result->fetch_assoc();
         $jasonArray["staff_name"]=$rowArray["staff_name"];
         $jasonArray["staff_mobile"]=$rowArray["staff_mobile"];
         $jasonArray["staff_email"]=$rowArray["staff_email"];
         $jasonArray["staff_doj"]=$rowArray["staff_doj"];
         $jasonArray["staff_image"]=$rowArray["staff_image"];
         $jasonArray["userDesignation"]= getField($conn, $rowArray["mn_id"], 'master_name', 'mn_id', 'mn_name');
         $jasonArray["userDepartment"]= getField($conn, $rowArray["dept_id"], 'department', 'dept_id', 'dept_name');
      }
