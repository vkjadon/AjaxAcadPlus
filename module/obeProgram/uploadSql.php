  ...
  if (in_array($file_ext, $allowed_ext)) {
    if ($_POST["action"] == "uploadSubject") {
      ...
      while ($row = fgetcsv($file_data)) {
        ...
        $student_email = data_check($row[3]);
        $student_email = preg_replace("/[^a-zA-Z0-9@$.*+]+/", "", $student_email);
        //echo $po_statement;
        ...
        $records = $result->num_rows;
        if ($records == 0) {
          $sql = "INSERT INTO $tn_std (program_id, batch_id, student_rollno, student_name, student_email, student_sno, update_id, student_status) VALUES ('$myProg', '$myBatch', '$student_rollno', '$student_name', '$student_email', '$student_sno', '$myId', '0')";
          echo "Inserted";
        } else {
          $sql = "update $tn_std set student_name='$student_name', batch_id='$myBatch', program_id='$myProg', student_sno='$student_sno', student_email='$student_email' where student_rollno='$student_rollno'";
          echo "Updated ";
        }
        $result_insert = $conn->query($sql);
        if (!$result_insert) echo $conn->error;
