...
if (isset($_POST['action'])) {
	if ($_POST['action'] == 'addTest') {
		...
		for ($i = 0; $i < count($array["data"]); $i++) {
      	...
      	<div class="card-body mt-0 py-1">
				...
				if ($portion_count > 1) {
					...
					$icp=1;	// To be used for Check Point Addition and Updating
					$sqlCP = "select * from qb_cp where qb_id='$id' order by qc_sno";
					$resultCP = $conn->query($sqlCP);
					while ($rows = $resultCP->fetch_assoc()) {
						if($icp=='1')echo '<h6 class="ml-0 mt-2 mb-0 p-0">Check Points</h6>';
						$qc_name = $rows["qc_name"];
						echo '<div class="row"><div class="col-9 p-0">
						<input type="text" class="form-control form-control-sm" id="checkPoint" value="'.$qc_name.'">
						</div>
						<div class="col-3 p-0">&nbsp;
					<a href="#" class="updateCP" data-sno="'.$icp.'" data-qb="' . $id . '"><i class="fa fa-edit"></i></a>
					<a href="#" class="trashCP" data-sno="'.$icp.'" data-qb="' . $id . '"><i class="fa fa-trash"></i></a>
					</div>
					</div>';
						$icp++;
					}
					...
					<div class="col-1 p-0"><br>&nbsp;
					<a href="#" class="addCheckPoint" data-sno="'.$icp.'" data-qb="' . $id . '"><i class="fa fa-plus"></i></a>
					</div>
					</div>';
				} else {
		...
		$qc_sno = $_POST['qc_sno'];
		$sql = "insert into qb_cp (qb_id, qc_name, qc_sno) values('$qb_id','$qc_name', '$qc_sno')";
		$result = $conn->query($sql);
		if ($result) {
			echo "Added Successfully";
		} else {
