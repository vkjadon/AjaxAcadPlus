...
if (isset($_POST['action'])) {
  if ($_POST['action'] == 'sessionClassList') {
    ...
    for ($i = 0; $i < count($id); $i++) {
      echo '<div class="xlText text-primary">' . getField($conn, $id[$i], 'class', 'class_id', 'class_name') . '[';
      echo '<span class="xlText text-primary">' . getField($conn, $id[$i], 'class', 'class_id', 'class_section') . '</span>]</div>';
      echo '<table class="table list-table-xs">';
      for ($k = 0; $k < count($day); $k++) {
        echo '<tr><td><span class="largeText text-secondary">' . $day[$k] . '</span></td>';
        $classId = $id[$i];
        ...
        for ($j = 1; $j < 10; $j++) {
          ...
          if ($result && $result->num_rows > 0) {
            while ($rows = $result->fetch_assoc()) {
              ...
              echo '<span class="footNote">' . $subject_code . ' </span><b>[' . $tlg_type . 'G-' . $tl_group . ']</b><br><span class="smallText text-primary">' . $staff_name . '</span>';
